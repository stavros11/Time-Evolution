...
class VariationalState():
  def __init__(self, state, dtype=tf.float64):
    self.dtype = dtype
    self.init_state = state[0][np.newaxis]
    self.real = tf.Variable(state[1:].real, dtype=dtype, trainable=True)
    self.imag = tf.Variable(state[1:].imag, dtype=dtype, trainable=True)
  
  def update(self, optimizer, complex_grads):
    grads = [complex_grads.real, complex_grads.imag]
    variables = [self.real, self.imag]
    optimizer.apply_gradients(zip(grads, variables))
    
  def numpy(self):
    state_t = self.real.numpy() + 1j * self.imag.numpy()
    return np.concatenate([self.init_state, state_t], axis=0)
...
for epoch in range(n_epochs):
  Ok, Ok_star_Eloc, Eloc, _ = en.all_states_gradient(var_state.numpy(), ham,
                                                     dt, Ham2=ham2)
